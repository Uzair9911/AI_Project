...
# returns coords of surrounding squares
    def diagonalSquares(self, coords):
        column,row = coords
        x1 = column + 1 #   right
        x2 = column - 1 #   left
        y1 = row + 1    #   down
...
#   given position of a piece (column, row) it would return all move both allowed and not allowed
    ...
    def availableMoves(self, coords, goOver):
        column,row = coords
    ...
    # Convert coordinates to integers before using as indices
        x = int(pieceCoords[0])
        y = int(pieceCoords[1])
        self.boardStat[y][x] = (black, pieceStat(None))
...
class game:
    ...
    def gameEvents(self):
        ...
        self.simpleMousePos = (int(self.mousePos[0] / self.wnd.squareSize), int(self.mousePos[1] / self.wnd.squareSize))
    ...
    def AiMove(self):
        ...
        piecesPos = list(updateAiPieces.keys())
    ...
    def undoMove(self,fromPos,fromStat,toPos, enemyRemoved):
        ...
        if enemyRemoved != None:
            enemyrow = int(enemyRemoved[1])
            enemycol = int(enemyRemoved[0])
            self.board.boardStat[enemyrow][enemycol] = (enemyRemoved[2], enemyRemoved[3])
    ...
    def ifJump(self, fromCoords, toCoords):
        ...
        # Convert to integers before using as indices
            x_int = int(x)
            y_int = int(y)
        
            removedColor = self.board.boardStat[y_int][x_int][0]
            removedStat = self.board.boardStat[y_int][x_int][1]
        
        # Still return the original floating point values if needed elsewhere
...
class startMenu:
    ...
    def drawButtons(self, pos, color):
        x,y = pos
    ...
    def addButtonText(self, pos, width, height, btn = "all"):
        x,y = pos
        if btn == 1 or btn == "all":
    ...
    def whichBtn(self, mouse, pos, width, height):
        x,y = pos
        btn = 0
        xPos = mouse[0]
        yPos = mouse[1]
    ...
    def redrawButton(self, mouse, pos, width, height):
        x,y = pos
        xPos = mouse[0]
        yPos = mouse[1]
